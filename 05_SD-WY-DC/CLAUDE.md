# Student Transportation Contract Rules — Set 5 (SD–WY + D.C.): Fan-Out Research Orchestrator

> **What this file is.** A Claude Code *dynamic-workflow orchestrator*. When triggered, it fans out
> **one research lane per jurisdiction (up to 11 lanes)**; each lane runs **research → adversarial validation**,
...
> run it. To research a single jurisdiction, say **“run jurisdiction &lt;name&gt;”**. Claude authors the orchestration script
...
(including school bus contracts) between public school districts (and D.C. public education authorities) and private contractors, for the eleven jurisdictions in
**Scope**, across the five **Fields**, and write the four **Deliverables**. Record what the governing authority
says; do not evaluate whether any jurisdiction’s framework is favorable or unfavorable to any party.

## Scope

**Jurisdictions — exactly these eleven (D.C. is included). This list is the single point of change when this orchestrator is reused for another batch:**

> District of Columbia, South Dakota, Tennessee, Texas, Utah, Vermont, Virginia, Washington, West Virginia, Wisconsin, Wyoming.

- **In scope:** the law of each listed jurisdiction governing the five Fields. Each jurisdiction is an **independent** research task.
- **Out of scope:** any jurisdiction not on the list (except where a source page incidentally mentions one);
  **cross-jurisdiction comparison or inference** (never use one jurisdiction’s rule to infer or fill a gap in another’s);
  ranking, scoring, or evaluating jurisdictions against each other.
- **As-of date:** the date the workflow executes. Every “current law” claim is anchored to that date.
- **District of Columbia:** treat the District’s instruments as the equivalent authorities — the D.C. Official Code (statutes), the D.C. Municipal Regulations (administrative code), D.C. Council acts/session laws, and OSSE / DCPS guidance; “school district” maps to DCPS and the public charter LEAs.
...
- **Fan-out — one lane per jurisdiction, `pipeline(jurisdictions, researchStage, validateStage)`.** Each jurisdiction flows through
  two staged agents independently, with **no barrier between stages** (jurisdiction B may be validating while jurisdiction C
  is still researching). Lanes are **isolated**: a lane sees only its assigned jurisdiction and never another lane’s work.
  - **Stage 1 — Research agent (one per jurisdiction).** Runs the **Per-jurisdiction research brief** for its assigned jurisdiction.
    Returns a schema-validated findings pack. **Does not write deliverable files.**
  - **Stage 2 — Adversarial-validation agent (one per jurisdiction).** Runs the **Adversarial-validation brief** against
...
- **Reduce step (orchestrator main thread, or a single dedicated final agent that runs *after* all lanes complete —
  never a lane inside the fan-out).** Folds the eleven validated packs into the four Deliverables and runs the
  **Synthesis** tasks. This is the only place deliverable files are written, and where completion is verified.
- **Concurrency & bounds.** Up to eleven lanes; the runtime auto-caps concurrent agents and pipelines the remainder.
  At most **one** re-research pass per rejected finding — no unbounded loops and no background polling. A lane
  returns when its jurisdiction is closed or its search budget is spent; it does not wait for further input.
- **Single-jurisdiction mode.** “Run jurisdiction &lt;name&gt;” runs one lane (research → validate) for that jurisdiction and the reduce
  step for that jurisdiction only, updating the four files in place.
- **Resilience.** Each lane writes its own evidence and reconciled pack to `scratch/<jurisdiction>.json` as it finishes,
  so a completed jurisdiction survives an interrupted run and a re-run can skip jurisdictions already done.

## Per-jurisdiction research brief (Stage 1 — runs for one assigned jurisdiction, `{JURISDICTION}`)

> You are a neutral legislative research analyst with live web search and filesystem access, assigned to **`{JURISDICTION}`
> only**. Your sole function is to identify and accurately record the content of `{JURISDICTION}`’s statutes, education
...
> research or compare any other jurisdiction. Record exactly what the governing authority says — do not infer, assume,
...
### Fields to document for `{JURISDICTION}`
...
### Special-education differentiation (every jurisdiction)

Check whether `{JURISDICTION}` law sets **different** term limits, rebid requirements, procurement rules, or renewal
...
1. Codified statute (the jurisdiction’s legislature/council or official code publisher).
2. Administrative code / education-department regulation.
3. Attorney-general opinion interpreting the relevant statute.
4. Official education-department / education-agency guidance or procurement manual citing a specific statute/regulation.
5. School-boards-association guidance or district policy manual citing a specific statute/regulation.
...
Start broad, then narrow to official sources. Adapt these queries to `{JURISDICTION}`: `school bus contract term limit` ·
...
support a term/renewal practice, record it as district practice or variance, **not** as the jurisdiction’s law. Preserve evidence as
...
Return a structured pack for `{JURISDICTION}`: the five fields (each with its value and the supporting citation),
special-education findings, an array of **source-ledger rows** (`Jurisdiction | Source Tier | Citation | Official URL | Code
...
## Adversarial-validation brief (Stage 2 — runs for the same assigned jurisdiction)

> You receive Stage 1’s pack for **`{JURISDICTION}`**. Your job is to try to **disprove** it, re-verify every citation, and
...
2. **Challenge every field.** Record each challenge in the challenge-matrix rows (`Jurisdiction | Finding Challenged |
...
Fold the eleven validated packs into the Deliverables. This step owns the deliverable’s evidentiary chain — do not staple
packs together; integrate them. Run all of:

1. **Per-jurisdiction assembly** — one table row per jurisdiction from its validated pack; assemble per-jurisdiction Notes, Exceptions, ledger
   rows, challenge rows, and verification notes.
2. **Coverage check** — every jurisdiction has all five fields populated (a documented `silent`/`not found`/`unknown` counts as
   populated); every unresolved field appears in **Exceptions / Unresolved Issues** with its negative search path.
3. **Source-validation revisit (load-bearing citations)** — for each jurisdiction, re-open the **primary statutory citation**
   ...
   silently retain it.
4. **Bias / neutrality strip** — the deliverables state **what the law is**, never whether a jurisdiction is favorable, better,
   or worse. Remove any evaluative or comparative framing that crept in.
5. **Cross-jurisdiction consistency sweep (format only)** — confirm uniform schema, labels, and cell formats across all eleven rows;
   flag outliers. This is a **formatting** check, not a legal comparison — it must not infer any jurisdiction’s rule from another’s.
...
for the eleven SD–WY + D.C. jurisdictions across the five fields), the neutral-recorder posture, and that each jurisdiction was researched
independently and adversarially validated. No cross-jurisdiction legal inference; no favorability judgments.]

### Per-jurisdiction findings index   (descriptive, one line per jurisdiction — not a comparison)
- **District of Columbia** — Primary citation: <official URL> · Source-validation: <verified/partially/not/unreachable> · Adversarial verdict: <confirmed/refuted/unresolved> · Confidence: <High/Medium/Low>
- … (one line for each of the eleven jurisdictions)

### Coverage & verify-and-converge
- Fields fully resolved vs. documented-silent/not-found/unknown: <descriptive QC tally>.
- Mode: adversarial — every jurisdiction’s five fields were challenged and every primary citation re-opened.
- Jurisdictions where the adversarial pass changed a finding: <list + what changed>. Jurisdictions where findings held: <list>.
```
...
Then: **Notes / Special Provisions** (one subsection per jurisdiction — small-purchase/dollar-threshold exemptions, sole-source/
...
Issues** (every jurisdiction/field not definitively answered, what was searched, what disconfirming searches were attempted, and
why it remains open — or a statement that all fields resolved); **Sources Consulted** (primary sources and official
guidance by jurisdiction, with official URLs; may summarize `source-ledger.md`).

**2. `source-ledger.md`** — one row per source reviewed, grouped by jurisdiction. Columns: `Jurisdiction | Source Tier | Citation |
...
**3. `verification-notes.md`** — per jurisdiction: the search strategy, the queries / official-site searches that mattered, the
...
orchestration provenance** (this run only): per jurisdiction, the lane status, the per-field validation verdict
(`confirmed`/`refuted`/`unresolved`), and which primary sources were re-verified vs. unreachable.

**4. `challenge-matrix.md`** — adversarial review, grouped by jurisdiction. Columns: `Jurisdiction | Finding Challenged | Disconfirming
Hypothesis | Challenge Searches / Sources | Result | Confidence Impact`. At least one row per jurisdiction for max term, rebid
requirement, renewal mechanics, special-education differentiation, and district variance.

## Guardrails

- **Neutrality.** Recorder of law, not advocate. No favorability, ranking, or cross-jurisdiction comparison anywhere in the
  deliverables; the only aggregate permitted is a descriptive QC tally derived from completed per-jurisdiction findings.
...
- **Bounded execution.** Exactly one re-research pass per rejected finding; no background polling; a lane returns when its
  jurisdiction is closed or its budget is spent.
...
2. **Confirm each jurisdiction's publication regime once, then stop hunting.** Some jurisdictions publish their official code **only** through a paid, state-sanctioned portal (e.g., LexisNexis) with **no free section-level deep link** (in batch `01_AL-GA`: Arkansas and Georgia). When that is the case, cite the **official portal landing page** plus the exact section in the citation text, add a one-line note in the source ledger, and read the verbatim text via a republication used only as a pathfinder — do not burn repeated tool calls searching for a free official deep link that does not exist.

3. **Fetch each official document once and read every needed section from it.** Several jurisdictions publish the code or administrative code as **title-level PDFs** (one PDF holds many sections) or as a single consolidated regulation filing (in batch `01_AL-GA`: Alaska's Title-14 statutes PDF and one DEED regulation PDF; Colorado's title-level CRS PDFs). When the sections you need share a containing document, fetch it **once** and extract every section from it, instead of one fetch per section.

4. **Minor.** In the reduce step, if lanes have written their `scratch/<jurisdiction>.json` packs (per the Resilience note), the final writer can **read those packs from disk** rather than receiving all eleven inline — trimming a large synthesis prompt with no loss of content. Match reasoning effort to stage as well: the research and adversarial-validation stages carry the hard reasoning and warrant full effort, while a narrow re-verification of an already-identified citation can run cheaper — but never lower effort on the research or validation stages.
...
```
phase('Research + validate (per jurisdiction)')
const JURISDICTIONS = ['District of Columbia','South Dakota','Tennessee','Texas','Utah','Vermont','Virginia','Washington','West Virginia','Wisconsin','Wyoming']
const packs = await pipeline(
  JURISDICTIONS,
  j => agent(researchBrief(j),  { label: `research:${j}`, schema: STAGE1_SCHEMA }),
  (draft, j) => agent(validateBrief(j, draft), { label: `validate:${j}`, schema: STAGE2_SCHEMA }),
)
...
`pipeline` (not `parallel`) is correct here: stages are per-jurisdiction and independent, so research and validation interleave
across jurisdictions with no barrier. Reserve a `parallel` barrier only if a future step must see all eleven jurisdictions at once.
